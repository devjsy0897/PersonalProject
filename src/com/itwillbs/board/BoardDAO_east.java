...
public class BoardDAO_east {
    ...
    public void insertBoard(BoardBean bb){
    	...
    	try {
			...
			sql = "select max(bno) from fun_board_east";
			...
			// (2) 글정보를 저장
			// sql
			sql = "insert into fun_board_east (bno,name,pass,subject,content,"
    ...
    public int getBoardCount(){
    	...
    	try {
			...
			sql = "select count(*) from fun_board_east";
			pstmt=con.prepareStatement(sql);
			//실행
			rs=pstmt.executeQuery();
	...
	public ArrayList getBoardList(){
		...
		try {
			...
			sql = "select * from fun_board_east "
	...
	// updateReadCount(bno)
		public void updateReadCount(int bno){
			...
			try {
				...
				sql = "update fun_board_east set readcount=readcount+1 "
						+ "where bno=?";
				pstmt = con.prepareStatement(sql);
		...
		public int updateBoard(BoardBean bb){
			...
			try {
				...
				// 글이 있는지 검색하는 sql 구문
				sql = "select pass from fun_board_east where bno=?";
				pstmt = con.prepareStatement(sql);
				...
				if(rs.next()){
					...
					if(bb.getPass().equals(rs.getString("pass"))){
					    ...
					    // 데이터 수정	
						// sql
						sql ="update fun_board_east set name=?,subject=?,content=? "
								+ "where bno=?";
						pstmt = con.prepareStatement(sql);
			...
			try {
				...
				if(rs.next()){
					if(pass.equals(rs.getString("pass"))){
						// 글삭제
						sql = "delete from fun_board_east where bno=?";
		...
		public void reInsertBoard(BoardBean bb){
			...
			try {
				...
				// re_seq 값을 1증가
				sql = "update fun_board_east set re_seq=re_seq+1 "
			    ...
			    /*************답글 추가 동작***************/
				sql="insert into fun_board_east "
						+ "values(?,?,?,?,?"
						+ ",?,?,?,?,now()"
						+ ",?,?,?)";
		...
		public ArrayList gallery(){
			...
			try {
				...
				sql="select bno, file from fun_board_east where file is not null";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
