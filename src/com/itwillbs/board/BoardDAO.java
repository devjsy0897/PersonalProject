	...
	// updateReadCount(bno)
		public void updateReadCount(int bno){
			...
			try {
				...
				sql = "update fun_board set readcount=readcount+1 "
						+ "where bno=?";
				pstmt = con.prepareStatement(sql);
		...
		public int updateBoard(BoardBean bb){
			...
			try {
				...
				// 글이 있는지 검색하는 sql 구문
				sql = "select pass from fun_board where bno=?";
				pstmt = con.prepareStatement(sql);
			...
			try {
				...
				if(rs.next()){
					if(pass.equals(rs.getString("pass"))){
						// 글삭제
						sql = "delete from fun_board where bno=?";
		...
		public void reInsertBoard(BoardBean bb){
			...
			try {
				...
				// re_seq 값을 1증가
				sql = "update fun_board set re_seq=re_seq+1 "
		...
		public ArrayList gallery(){
			...
			try {
				...
				sql="select bno, file from fun_board where file is not null";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
